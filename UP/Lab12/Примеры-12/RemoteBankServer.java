...
public class RemoteBankServer extends UnicastRemoteObject implements Bank.RemoteBank
{
	...
	throws RemoteException, Bank.BankingException
    {
        // Check if there is already an account under that name
        if (accounts.get(name) != null) 
            throw new Bank.BankingException("Account already exists.");
    ...
    Account verify(String name, String password) throws Bank.BankingException {
        synchronized(accounts) {
            ...
            if (acct == null) throw new Bank.BankingException("No such account");
            if (!password.equals(acct.password)) 
                throw new Bank.BankingException("Invalid password");
            return acct;
        }
    }
     ...
     **/
    public synchronized Bank.FunnyMoney closeAccount(String name, String password)
	throws RemoteException, Bank.BankingException
    {
        Account acct;
    ...
    public void deposit(String name, String password, Bank.FunnyMoney money) 
	throws RemoteException, Bank.BankingException
    {
    ...
    public Bank.FunnyMoney withdraw(String name, String password, int amount)
	throws RemoteException, Bank.BankingException
    {
        Account acct = verify(name, password);
        synchronized(acct) {
            if (acct.balance < amount) 
                throw new Bank.BankingException("Insufficient Funds");
            acct.balance -= amount;
